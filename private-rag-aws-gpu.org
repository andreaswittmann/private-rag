...
# Generate a new SSHkey pair specifically for RagFlow GPU
...
# Setup Swap Space for the machien
...
** GPU Instance Selection and Upgrading (Optional)
...
*** Python Development Tools Installation (Optional)
For Ragflow development only.
...
*** Development Environment Summary (Optional)
...
perl -i -pe 's/^  # /  / if /^  # /' docker-compose.yml
tail -n 25 docker-compose.yml
   
# Fix executor bug in docker-compose.yml, calling wrong entrypoint script
perl -i -pe 's{entrypoint: "/ragflow/entrypoint_task_executor.sh 1 3"}{entrypoint: "/ragflow/entrypoint.sh --disable-webserver --workers=2"}g' docker-compose.yml
cat docker-compose.yml | grep entrypoint
...
** Decommissioning the environment with Terraform
...
export PROJECT_DIR=~/Labs/private-rag/
cd "$PROJECT_DIR"
cd ..
pwd
rm -rf private-rag
